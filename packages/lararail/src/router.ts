import { route as getRoute } from 'ziggy-js'

import store from './store'
import { RouteCollection, Router } from './types'
...
export type RouteParameters<T extends RouteName> =
    | (Routes[T] extends { bindings: any }
          ? Partial<Record<keyof Routes[T]['bindings'], any>> &
                Record<string, any>
          : {})
    | string
    | number
...
export function route(name?: any, params?: any): any {
    ...
    return getRoute(name as any, params as any, absolute, {
        url,
        routes,
        defaults,
        port,
...
export function current<T extends RouteName>(
    name?: T,
